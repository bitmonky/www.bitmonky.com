...
if ($row = mkyMsFetch($result)){
  ...
  if ($row = mkyMsFetch($result)){
    ...
    <div ID="chanTop">
    <?php
    $source = getTRxt('A Channel Started By')." -  <a style='color:darkKhaki' href=\"javascript:wzGetPage('/whzon/mbr/mbrProfile.php?wzID=".$sKey."&fwzUserID=".$ownerID."');\">".$ownerName."</a>";
...
if ($hcoID){
  ...
  if ($hcRec){
    $hQry=$hcRec['hcoHash'];
    $source= "<a style='color:gold' href=\"javascript:wzGetPage('/whzon/public/homepg.php?wzID=".$sKey."&wzCID=".$inChatChanID."&fhQry=".mkyUrlEncode($hQry)."');\">View Channel Feed Here</a>";
    $imgAon="";
    $imgAoff="";
  }
}
...
if (!is_null($photoID)){
  ...
  $source= "<a href=\"javascript:wzGetPage('//".$whzdom."/whzon/mbr/mbrViewPhotos.php?wzID=".$sKey."&fwzUserID=".$phUserID."&vPhotoID=".$photoID."');\">View photo Here</a>";
  $imgAon="";
  $imgAoff="";
}
if ($imgAon == "")
  $imgAon="<a data='View ".$ownerName."`s Profile' alt='View ".$ownerName."`s Profile' href=\"javascript:wzGetPage('/whzon/mbr/mbrProfile.php?wzID=".$sKey."&fwzUserID=".$ownerID."');\">";
  $imgAoff = "</a>";
}
else {
  ?>
  <b style='font-size:13px;'>Talk About</b><b><font color='#6699ff'><?php sayTxt('Topic No Longer Available');?>:</font></b><br>
  <?php
}
if ($inChatChanID == $dateAppChan){
  $img = "<img src='//image.bitmonky.com/img/iconDating.png' style=''/>";
  $source= "<a style='color:darkKhaki;font-size:larger;font-weight:bold;' title='Open Dating App' href=\"javascript:wzGetPage('/whzon/apps/dating/appDating.php?wzID=".$sKey."');\">".getTRxt('MonkyTalk Dating App')."</a>";
...
if ($userID == $userID){
  ...
  echo "<div style='border:0px;border-radius: .5em;width:98%;margin-top:8px;margin-bottom:0px;background:#333333;padding:4px;'>";
}
else {
  $chatBoxBG = '#eeeeee';
  echo "<div style='border:0px;border-radius: .5em;width:99%;margin-top:8px;margin-bottom:0px;background:#fefefe;padding:4px;'>";
}
if (!isset($img)){$img ='';$imgAon='';$imgAoff='';}  
...
<?php
if ($userID != 0){
  echo "<input type='button' value='View Live Stream' onclick='chanGoLive()'/>";
  //}
  //if ($userID == 17621){
  echo " <input type='button' value='Broadcast' onclick='chanChooseBroadTime()'/>";
}
?>
</div>
<div align='right' ID='liveSCtrls' style='display:none'>
<input type='button' value=' Close Stream ' onclick='chanCloseStream()'/>
<input type='button' value=' View More '    onclick='chanMoreStreams()'/>
<input type='button' value='Broadcast'      onclick='chanChooseBroadTime()'/>
...
<input type='button' value='End Broadcast'  onclick='chanEndBroadcast()'/>
</div>
...
<?php if (safeGET('flogin')=="yes" && $userID==0 ){ ?>
...
?>    <b>This Channel Is Private:</b>
	  <?php if ($userID!=0 && $isBlock==0 ) { ?>
        <a style='color:darkKhaki' href='javascript:tRequestChanPass();'><?php sayTxt('Request Access To Room');?></a>
      <?php } ?>
      
      <p><?php sayTxt('Return To');?>:
	  <span style='background:lightSeaGreen;border-radius: .25em;FONT-FAMILY:padding:px;padding-left:3px;padding-right:3px;'><a style='color:white' href='javascript:wzChangeChannel(1);'><?php sayTxt('Lounge');?></a></span>
	  <span style='background:lightSeaGreen;border-radius: .25em;FONT-FAMILY:padding:px;padding-left:3px;padding-right:3px;'><a style='color:white' href='javascript:wzChangeChannel(293174);'><?php sayTxt('Chat');?></a></span>

	  </p>

    <?php } else { 
     $class = " style='color:#777777;background:".$chatBoxBG.";";
     $class .= "border-radius: .5em;FONT-FAMILY: tahoma,sans-serif;font-size:13px;font-weight:bold;padding:2px;width:calc(100% - 2px);height:50px;margin-top:5px;'";
...
<!--
     ...
     border-radius: .5em;FONT-FAMILY: tahoma,sans-serif;font-size:13px;font-weight:bold;padding:2px;width:98%;height:50px;margin-top:5px;'

    <?php if ($userID==0 ) { ?> style='font-weight:bold;font-size:13px;' placeholder='<?php echo $palmEmoj;?> ...<?php sayTxt('Type Here To Begin Chatting');?>!' onclick='wzQuickReg();' onkeypress='wzQuickReg();'<?php } else { if ($missingPInfo && $channel != 284267 ){ echo 'onclick=finishRegister();';} else {?>onkeypress='return <?php echo $action;?>'  placeholder='<?php echo $palmEmoj;?> ...<?php sayTxt('Type Here To Talk About This Topic');?>!' <?php }} ?>  NAME="fmsg"   WRAP='VIRTUAL' scrollbars='no'><?php echo $say;?></textArea>
-->
<br>
<style>
.unicode-button {
...
<span ID='showShareSpot' style='white-space:nowrap;'>
<?php if ($channel==1 ) { ?>
  <button type="button" value=" Send " onclick="sendMsg();" class="unicode-button" style="padding:.23em;"/> Send </button>
<?php } else { ?>
  <button type="button" value=" Send " onclick="sendMsg();" class="unicode-button"  style="padding:.23em;"/> Send </button>
<?php } ?>
<input alt='Share Photos With The Room' style='padding:.27em;font-size:1em;margin-top:6.5px;margin-bottom:6.5px;border-radius: .25em;' 
type="button" value="<?php sayTxt('Share Photo');?>" onclick='startPShare();'> 
<input ID='chanRestore' style='display:none;vertical-align:top;margin-top:.85em;' type='button' value=' + ' onclick='chanRestorMinimize()'/>
<input ID='chanMin' style='display:inline-block;vertical-align:top;margin-top:.85em;padding:.15em' type='button' value=' ^ ' onclick='chanMinimize()'/>
</span>
<?php } 
 ...
 if (!is_null($spoken)) { 
   ...
   <span style='font-size:smaller;'>
         ...
         <?php } else {?>
		   ...
		   <a data="<?php echo $whzdom;?> new voice chat channel... Try It!" href="javascript:wzGetPage('/whzon/mumble/howToMumble.php?wzID=<?php echo $sKey;?>');">
  ...
  if ($chanOwner ) { ?>
     ...
     <p/>
     <?php if (!is_null($privateChan) && $privateAccess==0 ) { ?>
