...
<body class='pgBody' style='margin:0px;'>
...
if ($tRec) {
  $pg=0;
    ...
    while ($tRec && $i<$nRows){
      ...
      if(!is_null($inMosh)) 
        $gotoMosh="<a title='Listening to music! Click to join them' href='javascript:parent.OpenGigBox(".$inMosh.");'><img style='border:0px;vertical-align:top;margin-top:3px;margin-left:5px;height:10px;width:13px;' src='//image.bitmonky.com/img/musicIcon.png'></a>";
      ...
      if ($isVshare == 1 || $isVshare == 2 ){
        ...
        $msg .= "<img style='vertical-align:top;float:right;margin-left:6px;width:40px;height:32px; border:0px solid #777777;' ";
        $msg .= "src='".$img."'></a>";
...
?>
      <table ID='cmsgEL<?php echo $msgID;?>' onmouseover='highlightChatMsg(<?php echo $msgID;?>);' onmouseout='normalizeChatMsg();' style='width:100%;margin:0px;margin-bottom:8px;border-collapse:collapse;border:0px solid;'><tr valign='top'><td style='width:50px;padding-left:0px;'>
      <a title='View <?php echo $firstname;?>`s Profile' href=javascript:parent.wzGetPage('/whzon/<?php echo $mpath;?>/mbrProfile.php?wzID=<?php echo $sKey;?>&fwzUserID=<?php echo $crUserID;?>',<?php echo $crUserID;?>);>
      <img alt='View Profile' style='width:<?php echo $iw;?>;height:<?php echo $ih;?>;border-radius:50%;margin:3px;margin-right:6px;border:0px solid #777777;' src='//image.bitmonky.com/<?php echo $imgfile;?>?id=<?php echo $crUserID;?>'></a>
      </td><td>
      <a style='font-size:12px;' title='View <?php echo $firstname;?>`s Profile' alt='View Profile'  href=javascript:parent.wzGetPage('/whzon/<?php echo $mpath;?>/mbrProfile.php?wzID=<?php echo $sKey;?>&fwzUserID=<?php echo $crUserID;?>',<?php echo $crUserID;?>);>
      ...
      <?php if ($userID!=0){
        ...
        <?php }else{?>
          <?php if ($userID!=$crUserID) {?>
            ...
            <a title='Send <?php echo $firstname;?> an email' href="javascript:parent.wzGetPage('/whzon/mbr/mail/sendMailForm.php?wzID=<?php echo $sKey."&fwzUserID=".$crUserID;?>');">
      ...
      <a href='javascript:parent.zoomConversation(<?php echo $crUserID;?>);'><img ID='zimgEL<?php echo $msgID;?>' Title='zoom in' src='//image.bitmonky.com/img/zoomBut.png' style='float:right;margin:1px;height:24px;width:24px;visibility:hidden;'></a>
      <br>
      <font style='color:white;font-size:13px;'><?php echo $msg;?><?php echo $gotoChan;?><?php echo $deleteMsg;?></font></font></td><td style='width:100px;'><?php echo $cDate;?></td></tr></table>
<?php
    ...
    echo "<p><a href='".$appName."?wzID=".$sKey."&newPg=".($nextPage + $nRows)."&fmbrID=".mkyUrlEncode($mbrID).'&fchanID='.$channel."'>Next</a>";
    if($nextPage > 0) {
      echo " | <a href='".$appName."?wzID=".$sKey."&newPg=".($nextPage - $nRows)."&fmbrID=".mkyUrlEncode($mbrID).'&fchanID='.$channel."'>Back</a>";
    }
    echo " | <a href='".$appName."?wzID=".$sKey."&newPg=0&fmbrID=".mkyUrlEncode($mbrID).'&fchanID='.$channel."'>Top</a>";
  }
