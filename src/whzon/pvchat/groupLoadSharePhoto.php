...
if (!$userID==0){
  if(isset($_POST['shareIt']) || 1==1) {
    ...
    if($imgsize > 0 && $imgsize < 28000000 && $status==0) {
      ...
      if ($mRec['nRec'] == 0){
        $SQL = "select sdate from ICDchat.tblChatSharedPic where md5ID = '".$md5ID."' and wzUserID=".$userID." and sgroupID=".$inGroupPChatID;
        $mRec = null;
        $myresult = mkyMyqry($SQL) or die($SQL);
        $mRec = mkyMyFetch($myresult);
        ...
        if ($mRec) {
          ...
          $SQL = "insert into ICDchat.tblChatSharedPic(wzUserID,shareID,sgroupID,img,sdate,thmnail,md5ID)";
          $SQL .= " values(".$userID.",0,".$inGroupPChatID.",'".$fullImg."','".$dstamp."','".$thmnail."','".$md5ID."')";
          $myresult = mkyMyqry($SQL);

          $SQL = "select sharePicID from ICDchat.tblChatSharedPic where md5ID = '".$md5ID."' and wzUserID=".$userID." and sgroupID=".$inGroupPChatID;
          $mRec = null;
          $myresult = mkyMyqry($SQL) or die($SQL);
          $mRec = mkyMyFetch($myresult);
...
$result=reportShare($userID,$inGroupPChatID,$dstamp);



function reportShare($userID,$sgroupID,$dstamp){
   $SQL = "Select sharePicID from ICDchat.tblChatSharedPic where wzUserID=".$userID." and sdate='".$dstamp."'";
   $myresult = mkyMyqry($SQL);
   $mRec = mkyMyFetch($myresult);
   $photoID=$mRec['sharePicID'];

   $sharetxt= "<table width=\"350\"><tr valign=\"top\"><td>has shared this photo with you<br></td><td width=\"87\">";
   $sharetxt .= "<a href=javascript:parent.pshare(".$photoID.",\"".mkyUrlEncode($dstamp)."\")>";
   $sharetxt .= "<img style=\"border:0px solid #777777\" ";
   ...
   $SQL = "INSERT INTO ICDchat.tblMbrChat (msg,msgUserID,msgMbrID,sentBy,groupID) values('".left($sharetxt,600)."',".$userID.",0,".$userID.",".$sgroupID.")";
   $myresult = mkyMyqry($SQL) or die($SQL);

   $sharetxt= "<table width=\"350\"><tr valign=\"top\"><td>has shared this photo with the group<br></td><td width=\"87\">";
   $sharetxt .= "<a href=javascript:parent.pshare(".$photoID.",\"".mkyUrlEncode($dstamp)."\")>";
   $sharetxt .= "<img style=\"border:0px solid #777777\" ";
   $sharetxt .= "src=\"//image.bitmonky.com/getPVPhotoTmn.php?id=".$photoID."&ck=".mkyUrlEncode($dstamp)."\"></a>";
